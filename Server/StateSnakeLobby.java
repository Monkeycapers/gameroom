 ...
 */
public class StateSnakeLobby extends Lobby {
    private User user;
    ...
    public StateSnakeLobby (User user, ChatContext chatContext) {
...
//                user.clientWorker.sendMessage(stringWriter.toString());
                ...
                for (Body b : snake.body) {
                    bodyList.add((int) (b.c.getRed() * 255) + "," + (int) (b.c.getGreen() * 255) + "," + (int) (b.c.getBlue() * 255) + "," + (b.xCord + 1) + "," + (b.yCord + 1));
                }

                StringWriter snakeWriter = new StringWriter();
                new JSONWriter(snakeWriter).object()
                        .key("head").value((255) + "," +  (255) + "," + (255) + "," + (snake.head.xCord + 1) + "," + (snake.head.yCord + 1))
                        .key("body").value(bodyList)
                        .key("direction").value(snake.direction.name())
                        .endObject();

                snakeList.add(snakeWriter.toString());

                StringWriter stringWriter = new StringWriter();
                new JSONWriter(stringWriter).object()
                        .key("argument").value("statelobbydraw")
                        .key("name").value(name)
                        .key("mapwidth").value(mapWidth)
                        .key("mapheight").value(mapHeight)
                        .key("list").value(snakeList)
                        .endObject();
